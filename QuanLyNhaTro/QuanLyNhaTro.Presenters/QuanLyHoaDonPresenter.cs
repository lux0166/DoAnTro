    ...
    public class QuanLyHoaDonPresenter
    {
        ...
        private void OnLoadHoaDons(object sender, EventArgs e)
        {
            try
            {
                ...
                var khachThues = _khachThueRepository.GetAll();

                var viewModel = hoaDons.Select(hd =>
                {
                    var hopDong = hd.HopDong; // Assuming HopDong is loaded with HoaDon
                    var phong = hopDong != null ? _phongRepository.GetById(hopDong.MaPhong) : null;
                    var khachThue = hopDong != null ? _khachThueRepository.GetById(hopDong.MaKhachThue) : null;
