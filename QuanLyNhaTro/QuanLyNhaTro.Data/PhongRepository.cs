...
namespace QuanLyNhaTro.Data
{
    ...
    {
        // Constructor that accepts a connection string (but ignores it to use DatabaseConnection helper,
        // or effectively we should use it if we want dependency injection.
        // For consistency with other repos that are parameterless in this file, we add this ctor to satisfy the Presenter).
        // However, the cleanest way is to use the passed string if provided, or fallback.
        // But since DatabaseConnection.GetConnection() reads from Config, let's stick to DatabaseConnection
        // and just allow the constructor signature to exist.
        ...
        public PhongRepository(string connectionString)
        {
