...
namespace QuanLyNhaTro.Data
{
    public class DichVuRepository
    {
        ...
        public DichVuRepository(string connectionString)
        {
        ...
        public DichVu GetByName(string tenDichVu)
        {
            DichVu service = null;
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                string query = "SELECT ID, TenDichVu, DonGia, DonViTinh, LoaiTinhPhi, TrangThai FROM DichVu WHERE TenDichVu = @TenDichVu";
                SqlCommand command = new SqlCommand(query, connection);
